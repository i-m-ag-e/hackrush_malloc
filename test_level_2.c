...
BenchmarkResult run_benchmark(int strategy, const char* name, int seed) {
    // Note: C's rand() produces a different sequence than Python's Mersenne
    // Twister, but the statistical distribution of the workload remains
    // identical.
    srand(seed);
    mem_init();
...
int main(void) {
    ...
    const char* winner = (ff.fragmentation_ratio <= bf.fragmentation_ratio)
                             ? "first_fit"
                             : "best_fit";
